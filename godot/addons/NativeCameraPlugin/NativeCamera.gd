...
signal frame_available(a_info: RefCounted)
...
@export var frame_height: int = FeedRequestScript.DEFAULT_HEIGHT
...
func create_feed_request() -> RefCounted:
	return (
		FeedRequestScript
		. new()
...
func start(a_request: RefCounted) -> void:
	if _plugin_singleton:
